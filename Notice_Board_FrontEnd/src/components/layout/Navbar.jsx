...
const Navbar = ({ username, userRole, onLogout, onCreateClick, onAdminClick }) => {
  ...
  // ... (Keep existing menuItems array and useEffect) ...
  const menuItems = [
    { id: 'home', label: 'Home Dashboard', icon: Home, color: 'text-sky-600', bg: 'bg-sky-50', link: '/', isExternal: false },
    { id: 'results', label: 'Exam Results', icon: Trophy, color: 'text-yellow-600', bg: 'bg-yellow-50', link: 'https://rtmnuresults.org/', isExternal: true },
    { id: 'buysell', label: 'Buy & Sell', icon: ShoppingBag, color: 'text-emerald-600', bg: 'bg-emerald-50', link: '/buy-sell', isExternal: false, comingSoon: false },
    { id: 'pyq', label: 'Previous Year Qs', icon: FileQuestion, color: 'text-purple-600', bg: 'bg-purple-50', comingSoon: true },
    { id: 'notes', label: 'Lecture Notes', icon: Book, color: 'text-blue-600', bg: 'bg-blue-50', comingSoon: true },
    { id: 'calendar', label: 'Academic Calendar', icon: Calendar, color: 'text-teal-600', bg: 'bg-teal-50', link: '/calendar', comingSoon: false },
    { id: 'labs', label: 'Virtual Labs', icon: FlaskConical, color: 'text-rose-600', bg: 'bg-rose-50', link: '/labs', isExternal: false },
    { id: 'community', label: 'Community', icon: Users, color: 'text-indigo-600', bg: 'bg-indigo-50', link: '/community', isExternal: false, comingSoon: true },
  ];
  ...
  return (
    <>
      ...
      <button
        type="button"
        // 1. CHANGED: duration-500 -> duration-200 (Backdrop Fade)
      ...
      {/* 2. CHANGED: duration-500 -> duration-200 (Slide In) */}
      <div className={`fixed top-0 left-0 h-full z-[70] w-[85%] md:w-[400px] bg-white/30 backdrop-blur-3xl border-r border-white/40 shadow-2xl transition-transform duration-200 cubic-bezier(0.4, 0, 0.2, 1) ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
